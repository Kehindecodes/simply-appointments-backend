...
export const notifyStaff = async  (date: string, time: string, serviceName: string, staffName: string, email: string): Promise<SentMessageInfo> => {
    const mailOptions = {
        from: process.env.EMAIL_USER,
        to: email,
        ...
        <html>
            ...
            <body>
                <div class="container">
                    ...
                    <p>Hello ${staffName},</p>
                    <p>You have a new appointment. Here are your appointment details:</p>
                    <div class="details">
                        <div class="detail-item"><span class="detail-label">Service:</span> ${serviceName}</div>
                        <div class="detail-item"><span class="detail-label">Attendant:</span> ${staffName}</div>
                        <div class="detail-item"><span class="detail-label">Appointment Date:</span> ${date}</div>
                    ...
                    <p>Kindly prepare for the appointment.</p>
                </div>
            </body>
        </html>
        `,
    ...
    try {
        const info = await transporter.sendMail(mailOptions);
        console.log(`Appointment update sent to ${email}`);
        return info;
    } catch (error) {
        console.error("Error sending appointment update: ", error);
        throw new Error(`Failed to send appointment update: ${error}`);
    }
}